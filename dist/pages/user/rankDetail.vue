...
<view class="weui-cells weui-cells_after-title">
            <view class="weui-cells weui-cells_after-title" wx:for="{{rank}}" wx:key="{{item.number}}">
               <navigator url="/pages/focus/describe?index={{item.number}}" class="weui-cell weui-cell_access" hover-class="weui-cell_active">
...
<script>
  ...
  export default class SmallClass extends wepy.page {
    ...
    onLoad(option) {
      ...
      wx.request({
        url: 'http://localhost/douyalocal2019.4.9/rank.php?rankName=' + option.index,
